...
class PromptReaderViewProvider implements vscode.WebviewViewProvider {
    public static readonly viewType = 'promptReaderView';
    ...
    // 保存提示词到文件
    private async savePromptToFile(content: string): Promise<void> {
        const config = vscode.workspace.getConfiguration('promptReader');
        const promptsPath = config.get<string>('promptsPath');
...
export function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        vscode.window.registerWebviewViewProvider(PromptReaderViewProvider.viewType, provider)
    );
    ...
    let addPromptDisposable = vscode.commands.registerCommand('prompt-reader.addPrompt', async () => {
        // 检查是否设置了提示词文件路径
        const config = vscode.workspace.getConfiguration('promptReader');
        let promptsPath = config.get<string>('promptsPath');
